...
Describe "Backup-Directories" {
    ...
    it "Backup copies 3 files to backup correctly" {
        ...
        "$BACKUP_DIR\$aFilePath" | Should -Exist
        Get-Content "$BACKUP_DIR\$aFilePath" | Should -Be $aFileContent
        "$BACKUP_DIR\$bFilePath" | Should -Exist
        Get-Content "$BACKUP_DIR\$bFilePath" | Should -Be $bFileContent
        "$BACKUP_DIR\$cFilePath" | Should -Exist
        Get-Content "$BACKUP_DIR\$cFilePath" | Should -Be $cFileContent
    }
    it "Updated files in target are copied over to the backup" {
        ...
        Get-Content "$BACKUP_DIR\$bFilePath" | Should -Be $bUpdatedContent
    }
    it "Removed files are scheduled for deletion" {
        ...
        Assert-MetaDeletedItem $META "$BACKUP_DIR\$aFilePath" | Should -BeTrue
        Assert-MetaDeletedItem $META "$BACKUP_DIR\$cFilePath" | Should -BeTrue
    }
    it "Restored files are unscheduled for deletion" {
        ...
        Assert-MetaDeletedItem $META "$BACKUP_DIR\$aFilePath" | Should -BeFalse
        Assert-MetaDeletedItem $META "$BACKUP_DIR\$cFilePath" | Should -BeTrue
    }
    it "Files that overwrite deleted files are unscheduled for deletion and updated" {
        ...
        Assert-MetaDeletedItem $META "$BACKUP_DIR\$cFilePath" | Should -BeFalse
    }
    it "Files that exceed their stale duration are deleted" {
        ...
        Assert-MetaDeletedItem $META "$BACKUP_DIR\$bFilePath" | Should -BeTrue

        Set-TimeDeleted $META "$BACKUP_DIR\$bFilePath" $expiredDT

        &"$APP_DIR\Backup-Directories.ps1" -Test -LogToFile

        "$BACKUP_DIR\$bFilePath" | Should -Not -Exist
        Assert-MetaDeletedItem $META "$BACKUP_DIR\$bFilePath" | Should -BeFalse
    }
    ...
    it "Directory in backup is deleted after staling" {
        ...
        $expiredDT = $deleted.TimeDeleted - $DELETE_TS
